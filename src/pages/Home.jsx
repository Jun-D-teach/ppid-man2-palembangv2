...
const Home = () => {
  ...
  const handleMenuPageClick = (e, page) => {
    e.preventDefault();

    // Semua submenu, termasuk INFORMASI PUBLIK, dibuka sebagai halaman CMS.
    // Kalau menu Informasi Publik diklik, tabel tengah ikut pindah ke kategori yang sama.
    if (isInformasiPublikPage(page)) {
      setActiveTab(getInfoCategoryKey(page));
    }
  ...
  const handleProsedurButton = () => {
    const prosedurPage = menuPages.find((page) => page.slug === "prosedur-permohonan");
    if (prosedurPage) {
      setSelectedMenuPage(prosedurPage);
      return;
    }
    setShowProsedur(true);
  };
                    ...
                    {nav.submenu.map((sub) => (
                      <li key={sub.id || sub.slug || sub.menu_label}>
                        <a
                          ...
                          className="block px-6 py-3 text-[11px] font-bold text-slate-600 hover:bg-emerald-50 hover:text-emerald-800 border-b border-gray-50 last:border-0"
                        >
                          {sub.menu_label}
                        </a>
                      </li>
            ...
            {activeInfoPage && (
              <div className="border-b border-emerald-100 bg-emerald-50/50 p-5 md:p-6">
                <div className="whitespace-pre-line text-sm leading-7 text-slate-600">
                  {activeInfoPage.isi || "Konten halaman ini belum diisi oleh admin."}
                </div>
        ...
        <div className="mx-auto max-w-7xl px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] font-bold uppercase tracking-widest">
              ...
              {parentOrder.slice(0, 4).map((parent) => (
                <button key={parent} onClick={() => setSelectedMenuPage(menuPages.find((page) => page.parent_menu === parent) || null)} className="text-left hover:text-white transition">
                  {parent}
                </button>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b pb-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-emerald-700">
                  {selectedMenuPage.parent_menu}
                </p>
                <h3 className="mt-1 text-2xl font-black text-slate-900">
                  {selectedMenuPage.judul || selectedMenuPage.menu_label}
                </h3>
              </div>

              <button
                onClick={() => setSelectedMenuPage(null)}
                className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-slate-100 text-xl font-black text-slate-500 hover:bg-red-50 hover:text-red-600"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-5 whitespace-pre-line text-sm leading-7 text-slate-600">
              {selectedMenuPage.isi || "Konten halaman ini belum diisi oleh admin."}
            </div>

            {selectedMenuPage.berkas_list?.length > 0 && (
              <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-slate-500">Berkas Terkait</p>
                <div className="space-y-2">
                  {selectedMenuPage.berkas_list.map((file) => (
                    <a
                      key={file.id}
                      href={getUploadUrl(file.nama_file)}
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between gap-3 rounded-xl bg-white px-4 py-3 text-xs font-bold text-slate-600 shadow-sm transition hover:text-emerald-700"
                    >
                      <span className="break-all">📄 {file.nama_asli || file.nama_file}</span>
                      <span className="shrink-0 rounded-full bg-emerald-100 px-3 py-1 text-[10px] font-black text-emerald-700">
                        {getFileFormat(file.nama_file)}
            ...
            {selectedMenuPage.slug === "prosedur-permohonan" && (
              <button
                onClick={handleAjukanClick}
                className="mt-6 w-full rounded-2xl bg-emerald-700 py-4 text-sm font-black uppercase text-white hover:bg-emerald-800"
              >
          ...
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b pb-4">
              <div>
                ...
                <h3 className="mt-1 text-2xl font-black text-slate-900">
                  {konten.prosedur_judul || "Prosedur Permohonan Informasi"}
                </h3>
              </div>

              <button
                onClick={() => setShowProsedur(false)}
                className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-xl font-black text-slate-500 hover:bg-red-50 hover:text-red-600"
              >
                ×
              </button>
            </div>

            <div className="mt-5 max-h-[60vh] overflow-y-auto text-sm leading-7 text-slate-600 whitespace-pre-line">
              {konten.prosedur_isi || "Belum ada prosedur layanan yang diinput oleh admin."}
            </div>
